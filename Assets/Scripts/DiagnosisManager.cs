...
public class DiagnosisManager : MonoBehaviour
{
    ...
    public void CheckDiagnosis()
    {
        ...
        if (!DiagnosticsManager.Instance.HasCompletedTests())
        {
            patientManager.TriggerNoTestReaction();
            return;
        }
    ...
    /* -------------------- UI Event Handlers -------------------- */
    private void OnDiseaseChanged() => selectedDisease = diseaseChoices.options[diseaseChoices.value].text;
    private void OnTreatmentChanged() => selectedTreatment = treatmentChoices.options[treatmentChoices.value].text;

    /* -------------------- Data Retrieval -------------------- */
    public string GetPatientSymptomsText() => string.Join(", ", patientSymptoms);
